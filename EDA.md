...
Plotting the distributions of `x`, `y`, and `z` after removing the outliers
displays the distributions more clearly. The right skewness of the these distributions 
makes sense; there should be more smaller diamonds than larger ones.
More notably, there spikes in the distribution at certain values. These are often near integer values, but not exactly on them. Without knowing more about diamond cutting, I can't
say more about what these spikes represent. But, I would guess that there are some commonly used sizes of diamonds, and these spikes are those. Also, I would guess that a diamond cut and carat value of a diamond imply values of `x`, `y`, and `z`. Since there are spikes in 
the distribution of carat sizes, and only a few different cuts, that results in these spikes.
